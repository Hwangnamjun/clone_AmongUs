...
public class CrewFloater : MonoBehaviour
{
    ...
    void Start()
    {
        for(int i = 0; i < 12; i++)
        {
            SpawnFloatingCrew((PlayerColorList)i, Random.Range(0, distance));
        }
    }
    ...
    void Update()
    {
        ...
        if(timer <= 0f)
        {
            SpawnFloatingCrew((PlayerColorList)Random.Range(0, 12), distance);
            timer = 0.5f;
        }
    }

    public void SpawnFloatingCrew(PlayerColorList colorList, float dist)
    {
        if(!crewStates[(int)colorList])
        {
            crewStates[(int)colorList] = true;
            ...
            crew.SetFloatingCrew(sprites[Random.Range(0, sprites.Count)], colorList, direction, floatingSpeed, rotateSpeed, Random.Range(0.5f, 1f));
        }
    }
